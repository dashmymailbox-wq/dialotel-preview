...
function vt_activate_plugin() {
	$defaults = array(
		// Page & SEO
		'vt_page_slug'        => 'compatibilite-amoureuse',
...
function vt_ensure_page_exists() {
	$slug  = get_option( 'vt_page_slug', 'compatibilite-amoureuse' );
	$title = get_option( 'vt_page_title', 'Compatibilite Amoureuse' );
	...
	if ( $existing ) {
		...
		if ( strpos( $existing->post_content, 'tirage_voyance' ) === false ) {
			wp_update_post( array(
				'ID'           => $existing->ID,
				'post_content' => '[tirage_voyance type="compatibilite-amoureuse"]',
			) );
		}
		return $existing->ID;
	}
